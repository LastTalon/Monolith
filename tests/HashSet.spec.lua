...
return function()
	...
	describe("Constructor", function()
		...
		it("should error when attempting to create a set from a non-table", function()
			expect(function()
				HashSet.new(true)
	...
	describe("Collection Interface", function()
		describe("Required Methods", function()
			...
			describe("Count", function()
				...
				it("should count the number of elements in the set", function()
					local set = HashSet.new({ 1, 2, 3, 4, 5 })
	...
	describe("HashSet Interface", function()
		describe("Required Methods", function()
			...
			describe("ProperSubsetOf", function()
				...
				it("should be false with an element and does not match the element of the other", function()
					local set = HashSet.new({ 1 })
					local other = HashSet.new({ 2 })
					expect(set:ProperSubsetOf(other)).to.equal(false)
				end)

				it("should be false with an element and matches the element of the other", function()
					local set = HashSet.new({ 1 })
					local other = HashSet.new({ 1 })
					expect(set:ProperSubsetOf(other)).to.equal(false)
				end)
				...
				it("should be false with multiple elements and the other contains a single element", function()
					local set = HashSet.new({ 1, 2, 3 })
					local other = HashSet.new({ 1 })
					expect(set:ProperSubsetOf(other)).to.equal(false)
				end)
			...
			describe("ProperSupersetOf", function()
				...
				it(
					"should be false with multiple elements and the other contains a single different element",
					function()
						local set = HashSet.new({ 1, 2, 3, 4, 5 })
				...
				it(
					"should be true with multiple elements and the other contains a single contained element",
					function()
						local set = HashSet.new({ 1, 2, 3, 4, 5 })
						local other = HashSet.new({ 1 })
						expect(set:ProperSupersetOf(other)).to.equal(true)
			...
			describe("SubsetOf", function()
				...
				it("should be true with an element and matches the element of the other", function()
					local set = HashSet.new({ 1 })
					local other = HashSet.new({ 1 })
					expect(set:SubsetOf(other)).to.equal(true)
				end)
			...
			describe("SetEquals", function()
				...
				it("should be false if both contain a single distinct element", function()
					local set = HashSet.new({ 1 })
					local other = HashSet.new({ 2 })
					expect(set:SetEquals(other)).to.equal(false)
				end)

				it("should be true if both contain a single identical element", function()
					local set = HashSet.new({ 1 })
					local other = HashSet.new({ 1 })
					expect(set:SetEquals(other)).to.equal(true)
				end)

				it("should be false if both contain a multiple distinct element", function()
					local set = HashSet.new({ 1, 2, 3, 4, 5 })
					local other = HashSet.new({ 1, 2, 3, 6, 7 })
					expect(set:SetEquals(other)).to.equal(false)
				end)

				it("should be true if both contain a multiple identical element", function()
					local set = HashSet.new({ 1, 2, 3, 4, 5 })
					local other = HashSet.new({ 1, 2, 3, 4, 5 })
					expect(set:SetEquals(other)).to.equal(true)
				end)
